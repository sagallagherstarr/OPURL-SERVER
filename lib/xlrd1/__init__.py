...
<p>(2) Excel for Windows stores dates by default as the number of days (or
fraction thereof) since 1899-12-31T00:00:00. Excel for Macintosh uses a default
start date of 1904-01-01T00:00:00. The date system can be changed in Excel on a
per-workbook basis (for example: Tools -> Options -> Calculation, tick the '1904
date system' box). This is of course a bad idea if there are already dates in
the workbook. There is no good reason to change it even if there are no dates in
the workbook. Which date system is in use is recorded in the workbook. A
workbook transported from Windows to Macintosh (or vice versa) will work
correctly with the host Excel. When using this module's xldate_as_tuple function
to convert numbers from a workbook, you must use the datemode attribute of the
Book object.  If you guess, or make a judgement depending on where you believe
the workbook was created, you run the risk of being 1462 days out of kilter.</p>

<p>Reference:
http://support.microsoft.com/default.aspx?scid=KB;EN-US;q180162</p>


<p>(3) The Excel implementation of the Windows-default 1900-based date system
works on the incorrect premise that 1900 was a leap year. It interprets the
number 60 as meaning 1900-02-29, which is not a valid date. Consequently any
number less than 61 is ambiguous. Example: is 59 the result of 1900-02-28
entered directly, or is it 1900-03-01 minus 2 days? The OpenOffice.org Calc
program 'corrects' the Microsoft problem; entering 1900-02-27 causes the number
59 to be stored. Save as an XLS file, then open the file with Excel -- you'll
see 1900-02-28 displayed.</p>

<p>Reference: http://support.microsoft.com/default.aspx?scid=kb;en-us;214326</p>

<p>(4) The Macintosh-default 1904-based date system counts 1904-01-02 as day 1
and 1904-01-01 as day zero. Thus any number such that (0.0 <= number < 1.0) is
ambiguous. Is 0.625 a time of day (15:00:00), independent of the calendar, or
should it be interpreted as an instant on a particular day
(1904-01-01T15:00:00)? The xldate_* functions in this module take the view that
such a number is a calendar-independent time of day (like Python's datetime.time
type) for both date systems. This is consistent with more recent Microsoft
...
<p>(5) Usage of the Excel DATE() function may leave strange dates in a
spreadsheet. Quoting the help file, in respect of the 1900 date system: 'If year
is between 0 (zero) and 1899 (inclusive), Excel adds that value to 1900 to
calculate the year. For example, DATE(108,1,2) returns January 2, 2008
(1900+108).'  This gimmick, semi-defensible only for arguments up to 99 and only
in the pre-Y2K-awareness era, means that DATE(1899, 12, 31) is interpreted as
3799-12-31.</p>

<p>For further information, please refer to the documentation for the xldate_*
functions.</p>

<h3> Named references, constants, formulas, and macros</h3>

<p> A name is used to refer to a cell, a group of cells, a constant value, a
...
figures are in different cells in different sheets, the user may define the name
`Sales` in each sheet. There are built-in names, like `Print_Area` and
`Print_Titles`; these two are naturally local to a sheet. </p>

<p> To inspect the names with a user interface like MS Excel, OOo Calc, or
Gnumeric, click on Insert/Names/Define. This will show the global names, plus
those local to the currently selected sheet. </p>

<p> A Book object provides two dictionaries (name_map and name_and_scope_map)
and a list (name_obj_list) which allow various ways of accessing the Name
objects. There is one Name object for each NAME record found in the workbook.
Name objects have many attributes, several of which are relevant only when
obj.macro is 1. </p>

<p> In the examples directory you will find namesdemo.xls which showcases the
many different ways that names can be used, and xlrdnamesAPIdemo.py which offers
3 different queries for inspecting the names in your files, and shows how to
extract whatever a name is referring to. There is currently one `convenience
method`, Name.cell(), which extracts the value in the case where the name refers
to a single cell. More convenience methods are planned. The source code for
Name.cell (in __init__.py) is an extra source of information on how the Name
attributes hang together. </p>
...
<p>A colour is represented in Excel as a (red, green, blue) (`RGB`) tuple with
each component in range(256). However it is not possible to access an unlimited
number of colours; each spreadsheet is limited to a palette of 64 different
colours (24 in Excel 3.0 and 4.0, 8 in Excel 2.0). Colours are referenced by an
index (`colour index`) into this palette.
...
0x19 (BIFF3-BIFF4), 0x41 (BIFF5-BIFF8): System window background colour for
pattern background (used in XF and CF records )<br />
...
2011-12-17, Phillip M. Feldman:

I patched the code to make it use the standard `warnings` module for warnings.
(This allows the calling program to suppress warnings that the user doesn't
want to see).

I added a `__sizeof__` method so that `sys.getsizeof` will return a correct
estimate of the memory occupied by a given `book` object.
...
def open_workbook(filename=None, logfile=sys.stdout, verbosity=0,
  ...
  encoding_override=None, formatting_info=False, on_demand=False):
    ...
    # Store the name of the file as an attribute of the workbook object:
    ...
    if biff_version <= 40:
        ...
        if on_demand:
           warnings.warn("*** WARNING: on_demand is not supported for this "
    ...
    if biff_version == 45 and bk.nsheets > 1:
        warnings.warn("*** WARNING: Excel 4.0 workbook (.XLW) file contains %d "
          "worksheets.\n"  "*** Book-level data will be that from the last "
          "worksheet.\n" % bk.nsheets)
    if not on_demand:
...
class Book(BaseObject):
    ...
    # Time in seconds to extract the XLS image as a contiguous string (or mmap
    # equivalent).
    load_time_stage_1 = -1.0

    # Time in seconds to parse the data from the contiguous string (or mmap
    # equivalent).
    load_time_stage_2 = -1.0

    def __sizeof__(self):
       """
       Report the memory occupied by the current book by applying
       `sys.getsizeof` to every attribute of this instance.  Note that this
       over-estimates the true memory footprint if different attributes refer to
       the same object, but it appears that `book` doesn't do this, so we should
       be OK.
       """
       s= 0
       for k in self.__dict__.keys():
          s+= sys.getsizeof(self.__dict__[k])
       return s
    ...
    def handle_boundsheet(self, data):
        ...
        if sheet_type != XL_BOUNDSHEET_WORKSHEET:
            ...
            warnings.warn("NOTE *** Ignoring non-worksheet data named %r (type "
    ...
    def derive_encoding(self):
        if self.encoding_override:
            ...
            if self.biff_version < 80:
               warnings.warn("*** No CODEPAGE record, no encoding_override; "
                 "will use 'ascii'\n")
               self.encoding= 'ascii'
            else:
