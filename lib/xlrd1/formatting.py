...
# 2010-12-17, Phillip M. Feldman: I patched the code to make it use the standard
# `warnings` module for all warnings.  (This allows the calling program to
# suppress warnings that the user doesn't want to see).
...
def is_date_format_string(book, fmt):
    ...
    if date_count:
       warnings.warn("is_date_format: ambiguous d=%d n=%d fmt='%r'" %
         (date_count, num_count, fmt))

    elif not got_sep:
       warnings.warn("The format '%s' produces a constant result." % fmt)
...
def handle_format(self, data, rectype=XL_FORMAT):
    ...
    if not(fmtkey > 163 or bv < 50):
        ...
        if 0 < fmtkey < 50 and (is_date_c ^ is_date_s):
            DEBUG = 2
            warnings.warn("Conflict between std format key %d and its format "
              "string %r\n" % (fmtkey, unistrg))
    if DEBUG == 2:
        warnings.warn("ty: %d; is_date_c: %r; is_date_s: %r; fmt_strg: %r" %
